        ...
        for module in extremities:
            for head in range(module.head_number):
                ...
                if new_module is not None:
                    ...
                    v_groups[resolution].append((list(range(curr_verts_number, curr_verts_number+len(new_module.verts))), new_module.base_radius / root.base_radius))
...
#     rot_3 = Matrix.Rotation(spin, 4, 'Z')
#     direction = ((rot_1 * Vector((0, 0, 1))) * rot_3) * rot_2
#     return np.asarray(direction.to_tuple())
#
#
# def average_vector(vectors):
#     """returns the average vector of a list of vectors"""
...
#
# class Split(Module):
#     def __init__(self, position=np.array([0.0, 0.0, 0.0]), direction=np.array([0.0, 0.0, 0.0]), radius=1, resolution=0, starting_index=0, spin=0, head_2_length=1, head_2_radius=.6):
#         Module.__init__(self, position, direction, radius, resolution, starting_index, spin)
#         self.type = 'split'
#         self.head_1_radius = .99 * self.base_radius
#         self.head_2_radius = head_2_radius * self.base_radius
...
#
#         spin_rotation = Matrix.Rotation(self.spin, 4, 'Z')
#         direction_rotation = Vector(self.direction).rotation_difference(Vector((0, 0, 1))).to_matrix()
#         self.verts = [((v * spin_rotation) * direction_rotation) + Vector(self.position) for v in self.verts]
#         self.verts = np.asarray([i.to_tuple() for i in self.verts])
#         # self.base_pos = [(v * spin_rotation) * direction_rotation + self.position for v in square(self.base_radius)]
#         uvs = [[(i / 4, uv_height), (i / 4, uv_height + .1*self.head_1_length / self.head_1_radius), ((i + 1) / 4, uv_height + .1*self.head_1_length / self.head_1_radius), ((i + 1) / 4, uv_height)] for i in range(3)]
#         uvs.extend([[(i / 4, uv_height), (i / 4, uv_height + .1*self.head_2_length / self.head_2_radius), ((i + 1) / 4, uv_height + .1*self.head_2_length / self.head_2_radius), ((i + 1) / 4, uv_height)] for i in range(4)])
...
#
# class Branch(Module):
#     def __init__(self, position=np.array([0.0, 0.0, 0.0]), direction=np.array([0.0, 0.0, 0.0]), radius=1, length=1, head_radius=.95, resolution=0,
#                  starting_index=0, spin=0):
#         Module.__init__(self, position, direction, radius, resolution, starting_index, spin)
#         self.type = "branch"
#         self.length = length
...
#         spin_rotation = Matrix.Rotation(self.spin, 4, 'Z')
#         direction_rotation = Vector(self.direction).rotation_difference(Vector((0, 0, 1))).to_matrix()
#         self.verts = np.asarray([((v * spin_rotation) * direction_rotation + Vector(self.position)).to_tuple() for v in self.verts])
#         # self.base_pos = [(v * spin_rotation) * direction_rotation + Vector(self.position) for v in square(self.base_radius)]
#         faces = [(i0, si, si + 1, i1), (i1, si + 1, si + 2, i2), (i2, si + 2, si + 3, i3), (i3, si + 3, si, i0)]
#         uvs = [[(i/4, uv_height), (i/4, uv_height + .1*self.length/self.base_radius), ((i+1)/4, uv_height + .1*self.length/self.base_radius), ((i+1)/4, uv_height)] for i in range(4)]
#
#         if self.head_module_1 is None:
...
#
# class Root(Module):
#     def __init__(self, position=np.array([0.0, 0.0, 0.0]), direction=np.array([0.0, 0.0, 0.0]), radius=1, resolution=0, starting_index=0, spin=0):
#         Module.__init__(self, position, direction, radius, resolution, starting_index, spin)
#         self.type = "root"
#         self.head_number = 1
#         self.head_1_radius = self.base_radius
...
#         spin_rotation = Matrix.Rotation(self.spin, 4, 'Z')
#         direction_rotation = Vector(self.direction).rotation_difference(Vector((0, 0, 1))).to_matrix()
#         self.verts = np.asarray([((v * spin_rotation) * direction_rotation + Vector(self.position)).to_tuple() for v in square(self.base_radius)])
#
#     def get_head_pos(self, head):
#         return self.position
#
